    ...
    if (addHair)
    {
      if (person is Girl)
      {
        if (person.SPAWN_noUglyHair)
        {
          ...
          person.StartingClothes.Insert(0, gameObjectList[Random.Range(0, gameObjectList.Count)]);
        }
        else if (person.SPAWN_onlyGoodHair)
        {
          ...
          person.StartingClothes.Insert(0, Main.Instance.Prefabs_Hair[Random.Range(0, Main.Instance.Prefabs_Hair.Count)]);
      }
      else
        person.StartingClothes.Insert(0, Main.Instance.Prefabs_MaleHair[Random.Range(0, Main.Instance.Prefabs_MaleHair.Count)]);
    }
    person.PersonType = this;
    if ((!this.DontColorMales || !(person is Guy)) && (!((Object) commingFrom != (Object) null) || !commingFrom.WithoutHairPaint) && this.HairColors != null && this.HairColors.Length != 0 && Random.Range(0, 100) > this.NaturalHairColorChance)
